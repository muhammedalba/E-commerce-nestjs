...
export class CarouselController {
  ...
  @ApiCreatedResponse({ description: 'The carousel item has been successfully created.', type: Carousel })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Bad Request. Invalid input data or missing images.' })
  ...
  @ApiOperation({ summary: 'Get all carousel items with optional filtering and pagination' })
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number for pagination' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Number of items per page' })
  @ApiQuery({ name: 'sort', required: false, type: String, description: 'Sort order (e.g., -createdAt)' })
  @ApiQuery({ name: 'fields', required: false, type: String, description: 'Comma-separated list of fields to include (e.g., description,carouselSm)' })
  @ApiQuery({ name: 'keyword', required: false, type: String, description: 'Search keyword for carousel items' })
  @ApiOkResponse({ description: 'Return all carousel items.', type: [Carousel] })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
  ...
  @ApiParam({ name: 'id', description: 'ID of the carousel item to retrieve', type: String })
  @ApiOkResponse({ description: 'Return a single carousel item.', type: Carousel })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Carousel item not found.' })
  ...
  @Patch(':id')
  @UseInterceptors(FileFieldsInterceptor(CarouselController.imageSize))
  @ApiOperation({ summary: 'Update an existing carousel item by ID (Admin only)' })
  @ApiBearerAuth()
  @ApiConsumes('multipart/form-data')
  @ApiBody({ type: UpdateCarouselDto })
  @ApiParam({ name: 'id', description: 'ID of the carousel item to update', type: String })
  @ApiOkResponse({ description: 'The carousel item has been successfully updated.', type: Carousel })
  ...
  @ApiBearerAuth()
  @ApiParam({ name: 'id', description: 'ID of the carousel item to delete', type: String })
  @ApiResponse({ status: HttpStatus.OK, description: 'The carousel item has been successfully deleted.' })
  @ApiResponse({ status: HttpStatus.UNAUTHORIZED, description: 'Unauthorized.' })
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: 'Forbidden. User does not have admin role.' })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Carousel item not found.' })
